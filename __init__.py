...
class PPB_PT_panel:																			# THe panel in the UI
	...
	def draw(self, context):     												# draw a user interface		(# align = True # use_slider = True # use_pin=true)
		...
		mycol = self.layout.column()
		mycol.scale_y = 2
		mycol.label(text="OUTPOST")
		mycol.operator("ue4_tools.create_pivot")
		mycol.scale_y = 1
...
class OUTPOST_RandomRemoveObject(bpy.types.Operator):
    bl_label = "RandomRemoveObject"
    bl_idname = "ue4_tools.random_remove_object"
    bl_description = "从选中的Object中随机移除一些Object, 仅保留输入的百分比所确定的Object数量"
    @classmethod
    def poll(cls, context):
        return context.mode == 'OBJECT'
    def execute(self, context):
        print("execute my create pivot method ")  
        pp:UE4_PivotPainterProperties = bpy.context.scene.pivot_painter
        number_removed = create_pivot.random_remove_selected_object(pp.percentageObjectToKeep)
        self.report({'INFO'}, "random remove object complete, removed num: " + str(number_removed))
        return {'FINISHED'}
...
classes = (
	...
	create_pivot.OUTPOST_CreatePivot,
	OUTPOST_RandomRemoveObject
)
